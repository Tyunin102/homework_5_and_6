...
# Моя метрика креативность
creativity_metric_geval = GEval(
    name="Creativity",
    criteria="Оцени, насколько последняя фраза (actual output) креативна. "
             "Учитывай неожиданность ответа, его логичность в контексте и оригинальность формулировки:",
    evaluation_params=[LLMTestCaseParams.INPUT, LLMTestCaseParams.ACTUAL_OUTPUT, LLMTestCaseParams.EXPECTED_OUTPUT]
)
# Моя метрика креативность
...
def get_model_predictions(gt_examples, system_message):
    predicted_examples = []
    print('ft:gpt-4o-mini-2024-07-18:personal::AgDTrkNt')
    for i_example in range(len(gt_examples)):
        completion = client.chat.completions.create(
            model="ft:gpt-4o-mini-2024-07-18:personal::AgDTrkNt",
...
def evaluate_metrics(gt_examples, predicted_examples1, predicted_examples2):
    ...
    for i_example in range(len(gt_examples)):
        ...
        # Моя метрика креативность
        creativity_metric_geval.measure(test_case1)
        predicted_examples1[i_example]['creativity_score'] = creativity_metric_geval.score
        scores1_creativity.append(creativity_metric_geval.score)

        creativity_metric_geval.measure(test_case2)
        predicted_examples2[i_example]['creativity_score'] = creativity_metric_geval.score
        scores2_creativity.append(creativity_metric_geval.score)
        # Моя метрика креативность
    return gt_examples, predicted_examples1, predicted_examples2, scores1, scores2, scores1_creativity, scores2_creativity
...
def main(args):
    ...
    for i_example in range(len(gt_examples)):
        ...
        print("-----------")
    gt_examples, predicted_examples1, predicted_examples2, scores1, scores2, scores1_creativity, scores2_creativity = evaluate_metrics(gt_examples,
                                                                                               predicted_examples1,
                                                                                               predicted_examples2)
    ...
    print("SCORE1 style:", np.mean(scores1), scores1)
    print("TTEST1 style:", ttest_res1)
    ttest_res2 = ttest_1samp(scores2, 0.5)
    print("SCORE2 style:", np.mean(scores2), scores2)
    print("TTEST2 style:", ttest_res2)

    # Моя метрика креативность
    print("SCORE1 creativity:", np.mean(scores1_creativity), scores1_creativity)
    print("TTEST1 creativity:", ttest_1samp(scores1_creativity, 0.5))
    print("SCORE2 creativity:", np.mean(scores2_creativity), scores2_creativity)
    print("TTEST2 creativity:", ttest_1samp(scores2_creativity, 0.5))

    rel_ttest_res = ttest_rel(scores1, scores2)

    print("=============== Rel Scores ======================")
    print("TTEST style:", rel_ttest_res)
    print("TTEST creativity:", ttest_rel(scores1_creativity, scores2_creativity))
    # Моя метрика креативность
